...
pub async fn change_block_type(
    ...
    // Determine the target block type
    let target_type = if let Some(t) = block_type {
        // Case 1: Direct type specification
        t
    } else if let Some(ext) = file_extension {
        // Case 2: Infer from extension
        infer_block_type(&ext).ok_or_else(|| {
            format!(
                "Cannot infer block type from extension '{}'. This extension is not supported (likely a binary file type)",
                ext
            )
        })?
    } else {
        ...
        return Err(
            "Either 'block_type' or 'file_extension' must be provided to change_block_type"
                .to_string(),
        );
    };
    ...
    log::info!(
        "Changing block type for block {} in file {} to '{}'",
        block_id,
        file_id,
        target_type
    );

    // Create command for core.change_type capability
    let cmd = Command::new(
        editor_id,
        "core.change_type".to_string(),
        block_id,
        serde_json::json!({ "block_type": target_type }),
    );
