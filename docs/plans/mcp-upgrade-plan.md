...
> 创建日期: 2026-01-28
> 更新日期: 2026-01-30
> 状态: Complete (P0-P4 全部完成, P5 测试待验证)
...
**当前状态：** MCP 双模式已实现（嵌入模式 SSE + 独立模式 stdio）。
**已完成：** MCP 独立模式（stdio 传输，独立 Engine，Phase 2 Agent 集成所需）。
...
| `src-tauri/src/mcp/standalone.rs` | **新增** — 独立 MCP Server 入口 (CLI 解析 + 启动) | ✅ 已完成 |
| `src-tauri/src/mcp/stdio_transport.rs` | **新增** — stdio 传输层 | ✅ 已完成 |
| `src-tauri/src/engine/standalone.rs` | **新增** — 独立 Engine (WAL 模式, 无需 GUI) | ✅ 已完成 |
| `src-tauri/src/commands/reload.rs` | **新增** — GUI EventStore 重载 | ✅ 已完成 |
...
│   └── event_store.rs            # ✅ WAL 模式由 standalone.rs 启用
├── commands/
...
### 4.2 模式 B: 独立模式（✅ 已实现 — Phase 2 F4/F5）
...
| 文件/目录 | 操作 | 对应任务 | 状态 |
|-----------|------|----------|------|
| `src-tauri/src/mcp/standalone.rs` | **新建** | F4-01 | ✅ 独立 MCP Server 入口，CLI 参数解析 |
...
| `src-tauri/src/engine/actor.rs` | **修改** | F5-02 | ✅ 添加 ReloadState 消息和 reload_state_from_db() |
| `src-tauri/src/engine/mod.rs` | **修改** | F5-01 | ✅ 添加 `pub mod standalone` |
| `src-tauri/src/mcp/mod.rs` | **修改** | F4-01 | ✅ 添加 `pub mod standalone` 和 `pub mod stdio_transport` |
| `src-tauri/src/commands/mod.rs` | **修改** | F5-02 | ✅ 添加 `pub mod reload` |
| `src-tauri/Cargo.toml` | **修改** | F4-02 | ✅ 添加 `transport-io` feature 到 rmcp |
| `src-tauri/src/main.rs` | **修改** | F4-01 | ✅ CLI 参数解析，支持 `mcp-server` 子命令 |
| `src-tauri/src/lib.rs` | **修改** | F5-02 | ✅ 注册 `reload_events` Tauri 命令 |
...
- [x] 解析 CLI 参数：`elfiee mcp-server --elf {path}`
- [x] `main.rs` 添加子命令分发（GUI 模式 vs MCP Server 模式）

**stdio 传输 (F4-02):**
- [x] 实现 `mcp/stdio_transport.rs`
- [x] Cargo.toml 添加 `transport-io` feature
- [x] JSON-RPC over stdin/stdout 通信
...
**Engine 独立模式 (F5-01):**
- [x] 实现 `engine/standalone.rs`：为 MCP Server 创建独立 Engine
- [x] 打开 .elf 文件（解压到临时目录）
- [x] EventStore 启用 WAL 模式（PRAGMA journal_mode=WAL）
- [x] 构建 StateProjector，无需 GUI
- [x] 自动创建默认 editor ("mcp-agent") 并授予权限

**GUI EventStore 重载 (F5-02):**
- [x] 实现 `commands/reload.rs`：Tauri 命令 `reload_events()`
- [x] 从 EventStore 重新加载所有 Events
- [x] 重建 StateProjector（通过 actor ReloadState 消息）
- [ ] 广播 `state_changed` 事件到前端（推迟到前端集成时实现）

### 8.6 P5 - 测试与验证

- [x] MCP 端点可用性验证
- [x] `cargo check` 编译通过（嵌入模式 + 独立模式）
- [ ] 独立模式端到端测试
...
|-------------|-----------|---------|------|
| **F4-01** MCP Server 入口 | 5.1 | ✅ 已完成 | `elfiee mcp-server --elf {path}` CLI 命令 |
| **F4-02** MCP 协议实现 | 3.3 + 5.2 | ✅ 已完成 | SSE (嵌入模式) + stdio (独立模式) |
| **F4-03** execute_command tool | 2.2 | ✅ 已超额完成 | 26 个独立 Tools + `elfiee_exec` 通用入口 |
| **F5-01** Engine 独立模式 | 5.3 | ✅ 已完成 | engine/standalone.rs + WAL 模式 |
| **F5-02** GUI EventStore 重载 | 5.4 | ✅ 已完成 | reload_events() Tauri 命令 + actor ReloadState |
