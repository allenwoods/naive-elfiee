# Elfiee 开发流程

> 本文档面向所有 Elfiee 项目开发者（包括 AI 辅助开发），说明完整的开发流程、环境搭建、协作规范和常见任务指南。

## 🎯 快速开始

### 1️⃣ 环境搭建（新手必看）

```bash
# 安装依赖
pnpm install

# 启动开发服务器（前后端热重载 + 自动生成 bindings.ts）
pnpm tauri dev
```

### 2️⃣ 开发工作流（核心流程）

```
后端定义接口 (#[tauri::command] + #[specta])
    ↓
在 lib.rs 注册命令和类型
    ↓
重启 pnpm tauri dev → 自动生成 bindings.ts
    ↓
前端通过 TauriClient 调用
```

### 3️⃣ AI 辅助开发检查清单

**后端开发必查：**
- [ ] Payload 类型添加了 `#[derive(Type)]`
- [ ] 在 lib.rs 的 **debug 和 release** 都注册了
- [ ] 能力处理器注意 Read/Write 的 Entity 区别
- [ ] 使用 `create_event()` 而不是直接修改状态

**前端开发必查：**
- [ ] 组件只调用 `Zustand Actions`，不直接使用 `TauriClient`
- [ ] `TauriClient` 只在 `app-store.ts` 的 Actions 内部使用
- [ ] 没有手动编辑 `bindings.ts`
- [ ] Payload 类型从 `bindings.ts` 导入
- [ ] 在组件中使用 try-catch 处理 Action 错误
- [ ] 不直接修改状态对象，必须通过 Actions

---

## 目录

1. [环境搭建](#环境搭建)
2. [开发工作流](#开发工作流)
3. [前后端协作流程](#前后端协作流程)
4. [添加新功能](#添加新功能)（完整示例）
5. [测试与调试](#测试与调试)
6. [Git 工作流](#git-工作流)
7. [常见任务快速指南](#常见任务快速指南)（⭐ 快速参考）
8. [AI 辅助开发指南](#ai-辅助开发指南)（⭐ AI 必读）

---

## 环境搭建

### 1. 系统要求

**必需工具：**
- **Node.js** >= 18.0.0
- **pnpm** >= 8.0.0
- **Rust** >= 1.70.0 (通过 rustup 安装)
- **Git**

**操作系统支持：**
- Windows 10/11
- macOS 10.15+
- Linux (Ubuntu 20.04+, Fedora, etc.)

### 2. 安装步骤

**A. 安装 Rust**

```bash
# 使用 rustup 安装 Rust
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh

# 验证安装
rustc --version
cargo --version
```

**B. 安装 Node.js 和 pnpm**

```bash
# 使用 nvm (推荐)
nvm install 18
nvm use 18

# 安装 pnpm
npm install -g pnpm

# 验证安装
node --version
pnpm --version
```

**C. 克隆项目**

```bash
git clone https://github.com/your-org/elfiee.git
cd elfiee
```

**D. 安装依赖**

```bash
# 安装前端依赖
pnpm install

# 验证 Rust 依赖（会自动下载）
cd src-tauri
cargo check
cd ..
```

### 3. 运行开发服务器

```bash
# 启动开发模式（前端 + 后端热重载）
pnpm tauri dev
```

**首次运行时：**
- Rust 依赖会自动编译（可能需要几分钟）
- Tauri 会创建开发窗口
- 前端会在 `http://localhost:5173` 运行
- `src/bindings.ts` 会自动生成

### 4. IDE 配置

**推荐 IDE：**
- **前端**: VS Code + Volar + TypeScript
- **后端**: VS Code + rust-analyzer / RustRover

**VS Code 推荐扩展：**
```json
{
  "recommendations": [
    "rust-lang.rust-analyzer",
    "tauri-apps.tauri-vscode",
    "Vue.volar",
    "bradlc.vscode-tailwindcss",
    "esbenp.prettier-vscode"
  ]
}
```

---

## 开发工作流

### 1. 开发模式

**启动开发服务器：**

```bash
pnpm tauri dev
```

**发生了什么：**
1. Vite 启动前端开发服务器 (热重载)
2. Cargo 编译 Rust 后端 (debug 模式)
3. Tauri 创建应用窗口
4. tauri-specta 生成 `src/bindings.ts`
5. 前端连接到后端 Tauri IPC

**热重载：**
- **前端代码变更** → 浏览器自动刷新 (Vite HMR)
- **后端 Rust 代码变更** → 自动重新编译 + 应用重启

### 2. 构建生产版本

```bash
# 构建完整应用
pnpm tauri build

# 输出位置：
# - Windows: src-tauri/target/release/bundle/msi/
# - macOS: src-tauri/target/release/bundle/dmg/
# - Linux: src-tauri/target/release/bundle/deb/
```

### 3. 仅构建前端

```bash
# 构建前端静态文件
pnpm build

# 输出位置：dist/
```

### 4. 运行测试

**前端测试：**

```bash
# 运行所有前端测试
pnpm test

# 运行测试并监听变更
pnpm test:watch

# 生成覆盖率报告
pnpm test:coverage
```

**后端测试：**

```bash
cd src-tauri

# 运行所有测试
cargo test

# 运行特定测试
cargo test test_name

# 显示输出
cargo test -- --nocapture
```

### 5. 代码格式化和检查

**前端：**

```bash
# 格式化代码
pnpm format

# 检查 TypeScript 类型
pnpm typecheck

# 运行 ESLint
pnpm lint
```

**后端：**

```bash
cd src-tauri

# 格式化代码
cargo fmt

# 运行 Clippy (Rust 代码检查)
cargo clippy

# 自动修复
cargo clippy --fix
```

---

## 前后端协作流程

### 1. 架构分离原则

```
前端 (React/TypeScript)
    ↓ IPC 通信
后端 (Rust/Tauri)
```

**【核心规则】前端和后端严格分离：**
- 前端只能通过 `src/bindings.ts` 调用后端
- 后端通过 Tauri Commands 暴露接口
- 使用 tauri-specta 自动生成类型绑定

### 2. 添加新接口的协作流程

**场景：前端需要一个新功能，但后端没有提供接口**

#### 步骤 1：前端开发者检查现有接口

```typescript
import { commands } from '@/bindings';

// 查看所有可用命令
console.log(Object.keys(commands));
```

**如果接口不存在 → 请求后端添加**

#### 步骤 2：后端开发者添加 Tauri Command

**A. 定义命令** (`src-tauri/src/commands/my_module.rs`)

```rust
#[tauri::command]
#[specta]
pub async fn my_new_command(
    param1: String,
    param2: i32,
    state: State<'_, AppState>,
) -> Result<MyResult, String> {
    // 实现逻辑
    Ok(MyResult { data: "result".to_string() })
}
```

**B. 定义返回类型** (如果需要)

```rust
#[derive(Debug, Clone, Serialize, Deserialize, Type)]
pub struct MyResult {
    pub data: String,
}
```

**C. 在 lib.rs 中注册**

```rust
// Debug 模式
#[cfg(debug_assertions)]
let specta_builder = tauri_specta::Builder::<tauri::Wry>::new()
    .commands(tauri_specta::collect_commands![
        // ... 已有命令 ...
        commands::my_module::my_new_command,
    ])
    .typ::<commands::my_module::MyResult>()
    // ...
```

```rust
// Release 模式（必须与 debug 一致）
#[cfg(not(debug_assertions))]
let builder = builder.invoke_handler(tauri::generate_handler![
    // ... 已有命令 ...
    commands::my_module::my_new_command,
]);
```

#### 步骤 3：重新生成绑定

```bash
# 重启开发服务器
pnpm tauri dev
```

**自动发生：**
- `src/bindings.ts` 更新
- 新命令 `myNewCommand` 可用
- 新类型 `MyResult` 可用

#### 步骤 4：前端使用新接口

```typescript
import { commands, type MyResult } from '@/bindings';

const result = await commands.myNewCommand('param1', 42);
if (result.status === 'ok') {
  const data: MyResult = result.data;
  console.log(data);
}
```

### 3. 添加新能力的协作流程

**场景：需要为特定块类型添加新操作**

#### 步骤 1：后端定义 Payload 类型

```rust
// src/extensions/my_extension/mod.rs
#[derive(Debug, Clone, Serialize, Deserialize, Type)]
pub struct MyCapabilityPayload {
    pub field1: String,
    pub field2: i32,
}
```

#### 步骤 2：后端实现能力处理器

```rust
// src/extensions/my_extension/my_capability.rs
#[capability(id = "my_extension.do_something", target = "my_block_type")]
fn handle_my_capability(cmd: &Command, block: Option<&Block>) -> CapResult<Vec<Event>> {
    let block = block.ok_or("Block required")?;
    let payload: MyCapabilityPayload = serde_json::from_value(cmd.payload.clone())?;

    // 实现逻辑
    let event = create_event(
        block.block_id.clone(),
        cmd.cap_id.as_str(),
        json!({ "result": payload.field1 }),
        &cmd.editor_id,
        1,
    )?;

    Ok(vec![event])
}
```

#### 步骤 3：后端注册能力和类型

```rust
// src/capabilities/registry.rs
impl CapabilityRegistry {
    pub fn register_extensions(&mut self) {
        // ...
        self.register_capability(handle_my_capability());
    }
}

// src-tauri/src/lib.rs
let specta_builder = /* ... */
    .typ::<extensions::my_extension::MyCapabilityPayload>()
    // ...
```

#### 步骤 4：前端在 Zustand Store 中定义 Action

**在 `src/lib/app-store.ts` 中添加 Action：**

```typescript
import { TauriClient } from '@/lib/tauri-client';
import type { MyCapabilityPayload } from '@/bindings';

export const useAppStore = create<AppStore>((set, get) => ({
  // ... 其他 state 和 actions ...

  // 添加新的 Action
  executeMyCapability: async (fileId: string, blockId: string, field1: string, field2: number) => {
    // 构造 Payload
    const payload: MyCapabilityPayload = {
      field1,
      field2,
    };

    // 调用 TauriClient（只在 Action 内部使用）
    const events = await TauriClient.block.executeCommand(fileId, {
      cmd_id: crypto.randomUUID(),
      editor_id: get().activeEditorId || '',
      cap_id: 'my_extension.do_something',
      block_id: blockId,
      payload: payload as any,
      timestamp: new Date().toISOString(),
    });

    // 重新加载块以获取最新状态
    const updatedBlock = await TauriClient.block.getBlock(fileId, blockId);
    set((state) => {
      const newBlocks = new Map(state.blocks);
      newBlocks.set(blockId, updatedBlock);
      return { blocks: newBlocks };
    });

    return events;
  },
}));
```

**在组件中调用 Action：**

```typescript
import { useAppStore } from '@/lib/app-store';

function MyComponent({ fileId, blockId }: Props) {
  const executeMyCapability = useAppStore((state) => state.executeMyCapability);

  async function handleExecute() {
    try {
      await executeMyCapability(fileId, blockId, 'value', 42);
      toast({ title: '执行成功' });
    } catch (error) {
      toast({ title: '执行失败', description: error.message });
    }
  }

  return <Button onClick={handleExecute}>执行能力</Button>;
}
```

---

## 添加新功能

### 完整示例：添加 "图片块" 功能

**需求：支持图片块（image block），可以上传和显示图片**

#### 1. 后端：定义扩展

**A. 创建扩展目录**

```bash
mkdir -p src-tauri/src/extensions/image
touch src-tauri/src/extensions/image/mod.rs
touch src-tauri/src/extensions/image/image_write.rs
```

**B. 定义 Payload 类型** (`mod.rs`)

```rust
use serde::{Deserialize, Serialize};
use specta::Type;

/// Payload for image.write capability
#[derive(Debug, Clone, Serialize, Deserialize, Type)]
pub struct ImageWritePayload {
    /// Base64 encoded image data
    pub image_data: String,
    /// Image format (png, jpg, etc.)
    pub format: String,
    /// Alt text
    pub alt_text: Option<String>,
}

pub mod image_write;
pub use image_write::handle_image_write;
```

**C. 实现能力处理器** (`image_write.rs`)

```rust
use capability_macros::capability;
use crate::capabilities::core::{CapResult, create_event};
use crate::models::{Block, Command, Event};
use super::ImageWritePayload;
use serde_json::json;

#[capability(id = "image.write", target = "image")]
fn handle_image_write(cmd: &Command, block: Option<&Block>) -> CapResult<Vec<Event>> {
    let block = block.ok_or("Block required")?;
    let payload: ImageWritePayload = serde_json::from_value(cmd.payload.clone())
        .map_err(|e| format!("Invalid payload: {}", e))?;

    // 验证
    if payload.image_data.is_empty() {
        return Err("Image data cannot be empty".into());
    }

    // 构建新状态
    let mut new_contents = serde_json::Map::new();
    new_contents.insert("image_data".to_string(), json!(payload.image_data));
    new_contents.insert("format".to_string(), json!(payload.format));
    if let Some(alt) = payload.alt_text {
        new_contents.insert("alt_text".to_string(), json!(alt));
    }

    // 创建事件
    let event = create_event(
        block.block_id.clone(),
        cmd.cap_id.as_str(),
        json!(new_contents),
        &cmd.editor_id,
        1,
    )?;

    Ok(vec![event])
}
```

**D. 注册扩展**

```rust
// src/extensions/mod.rs
pub mod image;

// src/capabilities/registry.rs
use crate::extensions::image::handle_image_write;

impl CapabilityRegistry {
    pub fn register_extensions(&mut self) {
        // ...
        self.register_capability(handle_image_write());
    }
}

// src-tauri/src/lib.rs
let specta_builder = /* ... */
    .typ::<extensions::image::ImageWritePayload>()
    // ...
```

#### 2. 前端：创建 UI 组件

**A. 在 Zustand Store 中添加 Action** (`src/lib/app-store.ts`)

```typescript
import { TauriClient } from '@/lib/tauri-client';
import type { ImageWritePayload } from '@/bindings';

export const useAppStore = create<AppStore>((set, get) => ({
  // ... 其他 state 和 actions ...

  // 添加上传图片的 Action
  uploadImage: async (fileId: string, blockId: string, imageData: string, format: string, altText: string) => {
    const payload: ImageWritePayload = {
      image_data: imageData,
      format,
      alt_text: altText,
    };

    await TauriClient.block.executeCommand(fileId, {
      cmd_id: crypto.randomUUID(),
      editor_id: get().activeEditorId || '',
      cap_id: 'image.write',
      block_id: blockId,
      payload: payload as any,
      timestamp: new Date().toISOString(),
    });

    // 刷新块状态
    const updatedBlock = await TauriClient.block.getBlock(fileId, blockId);
    set((state) => {
      const newBlocks = new Map(state.blocks);
      newBlocks.set(blockId, updatedBlock);
      return { blocks: newBlocks };
    });
  },
}));
```

**B. 创建图片块编辑器组件** (`src/components/editor/ImageBlockEditor.tsx`)

```typescript
import { useState } from 'react';
import { useAppStore } from '@/lib/app-store';
import { Button } from '@/components/ui/button';
import { toast } from '@/hooks/use-toast';

interface ImageBlockEditorProps {
  fileId: string;
  blockId: string;
}

export function ImageBlockEditor({ fileId, blockId }: ImageBlockEditorProps) {
  const [uploading, setUploading] = useState(false);

  // 从 Store 获取块数据和 Action
  const block = useAppStore((state) => state.blocks.get(blockId));
  const uploadImage = useAppStore((state) => state.uploadImage);

  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    setUploading(true);

    try {
      // 读取文件为 Base64
      const reader = new FileReader();
      reader.onload = async (event) => {
        const base64 = event.target?.result as string;
        const imageData = base64.split(',')[1]; // 移除 data:image/png;base64, 前缀
        const format = file.type.split('/')[1];

        try {
          // 调用 Zustand Action
          await uploadImage(fileId, blockId, imageData, format, file.name);

          toast({ title: '上传成功', description: '图片已保存' });
        } catch (error) {
          toast({
            title: '上传失败',
            description: error.message,
            variant: 'destructive'
          });
        }
      };

      reader.readAsDataURL(file);
    } catch (error) {
      console.error('Upload failed:', error);
      toast({ title: '读取文件失败', description: error.message });
    } finally {
      setUploading(false);
    }
  };

  // 渲染图片
  const imageData = (block.contents as any).image_data;
  const format = (block.contents as any).format;

  return (
    <div className="space-y-4">
      {imageData ? (
        <img
          src={`data:image/${format};base64,${imageData}`}
          alt={(block.contents as any).alt_text || 'Image'}
          className="max-w-full"
        />
      ) : (
        <p>No image uploaded</p>
      )}

      <input
        type="file"
        accept="image/*"
        onChange={handleFileUpload}
        disabled={uploading}
      />
    </div>
  );
}
```

#### 3. 测试

**A. 后端测试** (`src-tauri/src/extensions/image/mod.rs`)

```rust
#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_image_write() {
        // 测试代码
    }
}
```

**B. 前端测试** (`src/components/editor/ImageBlockEditor.test.tsx`)

```typescript
import { render, screen } from '@testing-library/react';
import { ImageBlockEditor } from './ImageBlockEditor';

describe('ImageBlockEditor', () => {
  it('renders upload button', () => {
    // 测试代码
  });
});
```

#### 4. 提交代码

```bash
# 创建功能分支
git checkout -b feat/image-block

# 提交后端代码
git add src-tauri/
git commit -m "feat(backend): add image block extension"

# 提交前端代码
git add src/
git commit -m "feat(frontend): add image block editor UI"

# 推送并创建 PR
git push origin feat/image-block
```

---

## 测试与调试

### 1. 前端调试

**使用浏览器开发者工具：**

```typescript
// 在代码中添加断点
debugger;

// 使用 console
console.log('Debug:', data);
console.table(blocks);
console.error('Error:', error);
```

**在 Tauri 开发窗口中打开 DevTools：**
- 右键点击窗口 → "Inspect Element"
- 或按 `F12`

### 2. 后端调试

**使用 println! 调试：**

```rust
println!("Debug: {:?}", value);
eprintln!("Error: {}", error);
```

**使用 rust-analyzer 断点调试：**

1. 在 VS Code 中安装 CodeLLDB 扩展
2. 在代码中设置断点
3. 运行 "Debug Rust" 配置

**查看日志：**

```bash
# 运行时会输出到控制台
pnpm tauri dev
```

### 3. 集成测试

**测试完整的命令流程：**

```rust
#[tokio::test]
async fn test_image_block_workflow() {
    // 创建文件
    let file_id = create_test_file().await;

    // 创建图片块
    let block_id = create_block(file_id.clone(), "image").await;

    // 上传图片
    let payload = ImageWritePayload {
        image_data: "base64data...".to_string(),
        format: "png".to_string(),
        alt_text: Some("Test image".to_string()),
    };

    let result = execute_image_write(file_id.clone(), block_id.clone(), payload).await;
    assert!(result.is_ok());

    // 验证状态
    let block = get_block(file_id, block_id).await.unwrap();
    assert_eq!(block.block_type, "image");
}
```

---

## Git 工作流

### 1. 分支策略

```
main (生产环境)
  ↑
dev (开发环境)
  ↑
feature/xxx (功能分支)
bugfix/xxx (修复分支)
```

**【重要】不要直接在 main/dev 上开发**

### 2. 创建功能分支

```bash
# 从 dev 创建功能分支
git checkout dev
git pull origin dev
git checkout -b feat/my-feature

# 开发...

# 提交
git add .
git commit -m "feat: add new feature"

# 推送
git push origin feat/my-feature
```

### 3. Commit 规范

使用 [Conventional Commits](https://www.conventionalcommits.org/) 格式：

```
<type>(<scope>): <subject>

<body>

<footer>
```

**类型 (type)：**
- `feat`: 新功能
- `fix`: Bug 修复
- `docs`: 文档更新
- `style`: 代码格式（不影响功能）
- `refactor`: 重构
- `test`: 测试相关
- `chore`: 构建/工具配置

**示例：**

```bash
git commit -m "feat(editor): add image block support"
git commit -m "fix(backend): resolve permission check bug"
git commit -m "docs: update development guide"
```

### 4. 创建 Pull Request

1. 推送分支到远程
2. 在 GitHub 创建 PR（从 `feat/xxx` → `dev`）
3. 填写 PR 描述：
   - 功能说明
   - 测试情况
   - 截图（如果有 UI 变更）
4. 请求 Code Review
5. 合并后删除分支

---

## 常见任务快速指南

### 1. 添加新的 Tauri Command

```bash
# 1. 定义命令
# src-tauri/src/commands/my_module.rs

#[tauri::command]
#[specta]
pub async fn my_command() -> Result<String, String> {
    Ok("result".to_string())
}

# 2. 在 lib.rs 中注册（debug + release）

# 3. 重启开发服务器
pnpm tauri dev

# 4. 前端使用
import { commands } from '@/bindings';
await commands.myCommand();
```

### 2. 添加新的 Payload 类型

```rust
// 1. 定义类型
#[derive(Debug, Clone, Serialize, Deserialize, Type)]
pub struct MyPayload {
    pub field: String,
}

// 2. 在 lib.rs 中注册
.typ::<extensions::my_extension::MyPayload>()

// 3. 重启开发服务器
// 4. 前端导入
import type { MyPayload } from '@/bindings';
```

### 3. 添加新的能力

```rust
// 1. 定义 Payload
#[derive(Serialize, Deserialize, Type)]
pub struct MyCapPayload { /* ... */ }

// 2. 实现处理器
#[capability(id = "ext.cap", target = "type")]
fn handle_my_cap(cmd: &Command, block: &Block) -> CapResult<Vec<Event>> {
    // ...
}

// 3. 注册能力
impl CapabilityRegistry {
    pub fn register_extensions(&mut self) {
        self.register_capability(handle_my_cap());
    }
}

// 4. 注册类型（lib.rs）
.typ::<extensions::my_extension::MyCapPayload>()
```

### 4. 添加新的 UI 组件

```bash
# 1. 创建组件文件
touch src/components/my-component/MyComponent.tsx

# 2. 创建测试文件
touch src/components/my-component/MyComponent.test.tsx

# 3. 实现组件和测试

# 4. 导出组件
# src/components/my-component/index.ts
export { MyComponent } from './MyComponent';
```

### 5. 运行特定测试

```bash
# 前端：运行特定测试文件
pnpm test src/components/MyComponent.test.tsx

# 后端：运行特定测试
cd src-tauri
cargo test test_my_function
```

---

## AI 辅助开发指南

### 1. 给 AI 的上下文

当使用 AI 辅助开发时，提供以下上下文：

**前端开发：**
```
我正在开发 Elfiee 前端功能。请参考：
- 前端开发规范: docs/mvp/dev/前端开发规范.md
- 我只能通过 TauriClient 调用后端（禁止直接使用 commands）
- 不能手动编辑 bindings.ts（由 tauri-specta 自动生成）
- 所有 Payload 类型从 bindings.ts 导入
```

**后端开发：**
```
我正在开发 Elfiee 后端功能。请参考：
- 后端开发规范: docs/mvp/dev/后端开发规范.md
- 所有前端接口必须是 Tauri Command（添加 #[specta] 宏）
- 每个能力必须定义类型化 Payload（添加 #[derive(Type)]）
- 遵循事件溯源架构（通过 Event 记录状态变更）
- 注意 Read vs Write 能力的 Entity 字段区别
```

### 2. AI 开发新功能时的检查清单

**后端开发检查：**
- [ ] 定义了 Payload 类型（添加 `#[derive(Type)]`）
- [ ] Tauri Command 添加了 `#[specta]` 宏
- [ ] 在 lib.rs 的 debug 和 release 模式都注册了命令
- [ ] 在 lib.rs 注册了所有 Payload 类型
- [ ] 编写了单元测试
- [ ] 能力处理器使用 `#[capability]` 宏
- [ ] 在 CapabilityRegistry 中注册了能力

**前端开发检查：**
- [ ] 没有手动编辑 `src/bindings.ts`
- [ ] 使用 TauriClient 而不是直接调用 commands
- [ ] 正确处理 Result 类型
- [ ] 使用生成的 Payload 类型（从 bindings 导入）
- [ ] 不直接修改 Block 对象，使用命令更新
- [ ] 添加了错误处理和用户反馈

### 3. 常见 AI 错误

| 错误类型 | ❌ 错误做法 | ✅ 正确做法 |
|---------|-----------|-----------|
| **前端调用** | 直接导入后端代码 | 使用 TauriClient |
| **类型绑定** | 手动编辑 bindings.ts | 在后端定义 + lib.rs 注册 |
| **Payload** | 手动解析 JSON | 定义类型 + serde_json::from_value |
| **状态修改** | 直接修改对象 | 使用 create_event() 返回 Event |
| **Entity** | Read 用 block_id | Write=block_id, Read=editor_id |

**✅ AI 开发前必读：**
```
1. 前端只能通过 TauriClient 调用后端（禁止直接使用 commands）
2. 不要手动编辑 bindings.ts（由 tauri-specta 自动生成）
3. 所有类型必须在 lib.rs 的 debug 和 release 都注册
4. 能力处理器注意 Read/Write 的 Entity 区别
5. 使用 create_event() 而不是直接修改状态
```

---

## 总结

### 开发流程要点

1. **环境搭建** → 安装 Rust + Node.js + pnpm → 运行 `pnpm tauri dev`
2. **开发模式** → 前后端热重载 → bindings.ts 自动生成
3. **添加功能** → 后端定义接口 → 注册命令和类型 → 前端使用
4. **测试** → 编写单元测试和集成测试 → 运行测试确保正确
5. **提交代码** → 遵循 Git 工作流 → 创建 PR → Code Review → 合并

### 核心原则

1. **前后端严格分离** - 通过 Tauri IPC 通信
2. **自动类型生成** - 使用 tauri-specta 确保类型安全
3. **事件溯源** - 所有状态变更通过 Event 记录
4. **能力为中心** - 基于 CBAC 的权限模型
5. **测试驱动** - 编写测试确保代码质量

### 遇到问题时

1. **查看文档**:
   - [前端开发规范](./前端开发规范.md) - 前端开发详细指南
   - [后端开发规范](./后端开发规范.md) - 后端开发详细指南（包含关键概念）
   - [CLAUDE.md](../../CLAUDE.md) - 项目总览

2. **检查 bindings.ts**:
   ```bash
   # 重新生成 bindings
   pnpm tauri dev
   ```

3. **查看日志**:
   - 前端：浏览器开发者工具 Console
   - 后端：终端输出

4. **运行测试**:
   ```bash
   pnpm test          # 前端
   cd src-tauri && cargo test  # 后端
   ```

---

**相关文档：**
- [前端开发规范](./前端开发规范.md) - 前端开发详细指南
- [后端开发规范](./后端开发规范.md) - 后端开发详细指南（包含 CBAC、Payload Types、向量时钟等关键概念）
- [CLAUDE.md](../../CLAUDE.md) - 项目总览和 AI 辅助开发指南
- [EXTENSION_DEVELOPMENT.md](../../guides/EXTENSION_DEVELOPMENT.md) - 扩展开发详细指南
- [FRONTEND_DEVELOPMENT.md](../../guides/FRONTEND_DEVELOPMENT.md) - Tauri Specta 详细指南
