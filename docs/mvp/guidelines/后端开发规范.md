# Elfiee 后端开发规范

> 本文档面向 Elfiee 项目的后端开发者（包括 AI 辅助开发），定义了后端开发的核心原则、架构规范和最佳实践。

## 🎯 核心重点（必读）

### ⚠️ 五大硬性规则

1. **【强制】所有前端接口必须是 Tauri Command**
   - 添加 `#[tauri::command]` + `#[specta]` 宏
   - 在 lib.rs 的 **debug 和 release** 模式都要注册

2. **【强制】每个能力必须定义类型化 Payload**
   - 添加 `#[derive(Serialize, Deserialize, Type)]`
   - 在 lib.rs 中注册 `.typ::<MyPayload>()`

3. **【关键】Read vs Write 能力的 Entity 不同**
   - **Write**: `entity = block.block_id` (被修改的块)
   - **Read**: `entity = cmd.editor_id` (读取者)

4. **【强制】通过 Event 记录状态变更，不直接修改**
   - ✅ 使用 `create_event()` 返回 Event
   - ❌ 不直接修改 Block 对象

5. **【强制】CBAC 授权检查**
   - Owner 总是授权
   - 非 Owner 需要 Grant
   - 使用 `#[capability]` 宏自动生成授权

### 📋 快速查阅

```rust
// 1. 定义 Tauri Command
#[tauri::command]
#[specta]
pub async fn my_command(param: String) -> Result<MyResult, String> { }

// 2. 定义 Payload 类型
#[derive(Debug, Clone, Serialize, Deserialize, Type)]
pub struct MyPayload { pub field: String }

// 3. 定义能力处理器
#[capability(id = "ext.write", target = "block_type")]
fn handle_write(cmd: &Command, block: &Block) -> CapResult<Vec<Event>> {
    let payload: MyPayload = serde_json::from_value(cmd.payload.clone())?;

    let event = create_event(
        block.block_id.clone(),  // Write: entity = block_id
        format!("{}/{}", cmd.editor_id, cmd.cap_id),
        json!(new_contents),
        &cmd.timestamp,
    )?;

    Ok(vec![event])
}

// 4. 在 lib.rs 注册
.commands(collect_commands![my_command])
.typ::<MyPayload>()
```

---

## 目录

1. [核心原则](#核心原则)
2. [架构概览](#架构概览)
3. [Tauri Command 开发](#tauri-command-开发)
4. [能力系统开发](#能力系统开发)（⭐ 包含 Read/Write、Payload、CBAC）
5. [扩展开发](#扩展开发)
6. [事件溯源](#事件溯源)（⭐ 包含向量时钟）
7. [类型安全与自动生成](#类型安全与自动生成)
8. [测试规范](#测试规范)
9. [常见错误与陷阱](#常见错误与陷阱)

---

## 核心原则

### 1. 前端只能通过暴露的接口访问后端

**【硬性规则】所有前端可调用的功能必须定义为 Tauri Command**

后端代码运行在独立的 Rust 进程中，前端（WebView）无法直接访问。前端只能通过 Tauri IPC 调用后端。

**✅ 正确做法：定义 Tauri Command**

```rust
#[tauri::command]
#[specta]
pub async fn execute_command(
    file_id: String,
    cmd: Command,
    state: State<'_, AppState>,
) -> Result<Vec<Event>, String> {
    // 实现逻辑
}
```

**❌ 错误做法：定义普通函数但期望前端调用**

```rust
// ❌ 前端无法调用这个函数
pub fn some_utility(file_id: String) -> Result<Block, String> {
    // 实现逻辑
}
```

### 2. 使用 tauri-specta 自动生成类型

**【硬性规则】所有 Tauri Command 和相关类型必须使用 tauri-specta**

- 在 Tauri Command 上添加 `#[specta]` 宏
- 在数据类型上添加 `#[derive(Type)]` 宏
- 在 `src-tauri/src/lib.rs` 中注册命令和类型

### 3. 遵循事件溯源架构

**【核心架构】所有状态变更通过 Event 记录**

```
Command → 授权检查 → Capability Handler → Event → State Projection
```

- 不直接修改状态
- 所有修改通过生成 Event 实现
- State 由 Event 重放构建

### 4. 能力为中心的权限模型 (CBAC)

**【核心架构】基于能力的访问控制**

- 每个操作由 Capability 定义
- 授权通过 `certificator` 检查
- Block Owner 拥有所有权限
- 其他 Editor 需要 Grant 授权

---

## 架构概览

### 技术栈

- **语言**: Rust (Edition 2021)
- **框架**: Tauri 2.x
- **数据库**: SQLite (通过 rusqlite)
- **异步运行时**: Tokio
- **序列化**: serde + serde_json
- **类型生成**: tauri-specta + specta

### 目录结构

```
src-tauri/
├── src/
│   ├── main.rs                 # Tauri 入口
│   ├── lib.rs                  # 【关键】Tauri 命令注册 + Specta 配置
│   ├── models/                 # 数据模型
│   │   ├── mod.rs
│   │   ├── block.rs            # Block 定义
│   │   ├── command.rs          # Command 定义
│   │   ├── event.rs            # Event 定义
│   │   ├── editor.rs           # Editor 定义
│   │   ├── grant.rs            # Grant 定义
│   │   └── payloads.rs         # Core payload 类型
│   ├── commands/               # 【重要】Tauri Commands
│   │   ├── mod.rs
│   │   ├── file.rs             # 文件操作命令
│   │   ├── block.rs            # 块操作命令
│   │   ├── editor.rs           # 编辑器操作命令
│   │   ├── event.rs            # 事件操作命令
│   │   └── checkout.rs         # 工作区导出命令
│   ├── capabilities/           # 能力系统
│   │   ├── mod.rs
│   │   ├── core.rs             # 核心能力接口
│   │   ├── registry.rs         # 能力注册表
│   │   ├── grants.rs           # 授权表
│   │   └── builtins/           # 内置能力
│   │       ├── create.rs
│   │       ├── delete.rs
│   │       ├── link.rs
│   │       ├── grant.rs
│   │       └── ...
│   ├── extensions/             # 扩展系统
│   │   ├── mod.rs
│   │   ├── markdown/           # Markdown 扩展
│   │   │   ├── mod.rs
│   │   │   ├── markdown_write.rs
│   │   │   └── markdown_read.rs
│   │   ├── code/               # Code 扩展
│   │   └── directory/          # Directory 扩展
│   ├── engine/                 # Elfile Engine (事件溯源引擎)
│   │   ├── mod.rs
│   │   ├── actor.rs            # Engine Actor
│   │   ├── manager.rs          # Engine Manager
│   │   ├── event_store.rs      # Event Store (SQLite)
│   │   └── state.rs            # State Projector
│   ├── elf/                    # .elf 文件格式处理
│   │   ├── mod.rs
│   │   └── archive.rs          # ZIP 档案操作
│   ├── utils/                  # 工具函数
│   │   ├── mod.rs
│   │   ├── fs_scanner.rs
│   │   ├── path_validator.rs
│   │   └── time.rs
│   ├── state.rs                # 全局应用状态
│   └── config.rs               # 配置管理
├── Cargo.toml                  # Rust 依赖
├── tauri.conf.json             # Tauri 配置
└── build.rs                    # 构建脚本
```

---

## Tauri Command 开发

### 1. 定义 Tauri Command

**【规范】所有暴露给前端的函数必须：**
1. 添加 `#[tauri::command]` 宏
2. 添加 `#[specta]` 宏（用于类型生成）
3. 在函数上方写详细的文档注释
4. 返回 `Result<T, String>` 类型

**示例：**

```rust
use tauri::State;
use crate::models::{Block, Command, Event};
use crate::state::AppState;

/// Execute a command on a block in the specified file.
///
/// This is the primary way to modify blocks. Commands are processed by the engine actor,
/// which handles authorization, execution, and persistence.
///
/// # Arguments
/// * `file_id` - Unique identifier of the file containing the block
/// * `cmd` - Command to execute (create, delete, link, write, etc.)
///
/// # Returns
/// * `Ok(events)` - Events generated by the command execution
/// * `Err(message)` - Error description if execution fails
#[tauri::command]
#[specta]
pub async fn execute_command(
    file_id: String,
    cmd: Command,
    state: State<'_, AppState>,
) -> Result<Vec<Event>, String> {
    // 1. 获取 Engine Manager
    let manager = state.engine_manager.read().await;

    // 2. 获取文件的 Engine Actor
    let engine = manager
        .get_engine(&file_id)
        .ok_or_else(|| format!("File not open: {}", file_id))?;

    // 3. 发送命令到 Engine Actor
    engine.execute_command(cmd).await
}
```

### 2. 在 lib.rs 中注册命令

**【关键文件】`src-tauri/src/lib.rs`**

**必须在两个地方注册命令：**

1. **Debug 模式** (使用 tauri-specta)：

```rust
#[cfg(debug_assertions)]
let builder = {
    let specta_builder = tauri_specta::Builder::<tauri::Wry>::new()
        .commands(tauri_specta::collect_commands![
            // 文件操作
            commands::file::create_file,
            commands::file::open_file,
            commands::file::save_file,
            // 块操作
            commands::block::execute_command,
            commands::block::get_all_blocks,
            // ... 其他命令
        ])
        // 注册类型
        .typ::<models::Block>()
        .typ::<models::Command>()
        // ... 其他类型
        ;

    // 导出 TypeScript 绑定
    specta_builder
        .export(
            Typescript::default().bigint(BigIntExportBehavior::Number),
            "../src/bindings.ts",
        )
        .expect("Failed to export TypeScript bindings");

    builder.invoke_handler(specta_builder.invoke_handler())
};
```

2. **Release 模式** (使用标准 Tauri handler)：

```rust
#[cfg(not(debug_assertions))]
let builder = builder.invoke_handler(tauri::generate_handler![
    // 文件操作
    commands::file::create_file,
    commands::file::open_file,
    commands::file::save_file,
    // 块操作
    commands::block::execute_command,
    commands::block::get_all_blocks,
    // ... 其他命令（必须与 debug 模式一致）
]);
```

**【重要】两个列表必须完全一致！**

### 3. 命令命名规范

**Rust 函数命名：**
- 使用 `snake_case`
- 动词在前：`create_file`, `execute_command`, `get_block`

**前端自动生成命名：**
- 自动转换为 `camelCase`
- `create_file` → `createFile`
- `get_all_blocks` → `getAllBlocks`

---

## 能力系统开发

### 1. 能力架构

**Capability** 是 Elfiee 的核心概念，定义了可以对 Block 执行的操作。

**能力组成部分：**

```rust
pub struct Capability {
    pub id: String,                          // 能力 ID (e.g., "markdown.write")
    pub target_block_type: Option<String>,   // 目标块类型 (None = 所有类型)
    pub certificator: Box<dyn Certificator>, // 授权检查器
    pub handler: Box<dyn CapabilityHandler>, // 执行处理器
}
```

### 1.1 Read vs Write 能力的区别

**【关键概念】Write 和 Read 能力的 Event 结构不同**

能力分为两大类，它们在 Event 的 `entity` 字段上有本质区别：

#### Write 能力（修改状态）

**Entity = block.block_id**（被修改的块）

```rust
/// Write 能力：修改 Block 的内容
#[capability(id = "markdown.write", target = "markdown")]
fn handle_markdown_write(cmd: &Command, block: Option<&Block>) -> CapResult<Vec<Event>> {
    let block = block.ok_or("Block not found")?;
    let payload: MarkdownWritePayload = serde_json::from_value(cmd.payload.clone())?;

    let mut new_contents = serde_json::Map::new();
    new_contents.insert("markdown".to_string(), json!(payload.content));

    // ✅ Entity 是被修改的 Block
    let event = create_event(
        block.block_id.clone(),                      // ← Entity: 被修改的块
        format!("{}/{}", cmd.editor_id, cmd.cap_id), // Attribute
        json!(new_contents),                         // Value: 新状态
        &cmd.timestamp,
    )?;

    Ok(vec![event])
}
```

**为什么 Entity 是 block_id？**
- StateProjector 需要知道**哪个块被修改了**
- 通过重放事件，StateProjector 更新对应块的状态
- Entity 指向状态变更的目标对象

#### Read 能力（观察状态）

**Entity = cmd.editor_id**（读取者）

```rust
/// Read 能力：读取 Block 的内容（不修改）
#[capability(id = "markdown.read", target = "markdown")]
fn handle_markdown_read(cmd: &Command, block: Option<&Block>) -> CapResult<Vec<Event>> {
    let block = block.ok_or("Block not found")?;

    let markdown = (block.contents.as_object()
        .and_then(|obj| obj.get("markdown"))
        .and_then(|v| v.as_str())
        .unwrap_or(""))
        .to_string();

    // ✅ Entity 是读取者（Editor）
    let event = create_event(
        cmd.editor_id.clone(),                       // ← Entity: 读取者
        format!("{}/{}", cmd.editor_id, cmd.cap_id), // Attribute
        json!({
            "block_id": block.block_id,              // 哪个块被读取
            "markdown": markdown,                     // 读取的数据
        }),
        &cmd.timestamp,
    )?;

    Ok(vec![event])
}
```

**为什么 Entity 是 editor_id？**
- Read 操作不修改 Block 状态
- Event 记录的是**谁读取了什么**（审计日志）
- Entity 指向执行读取的 Editor

#### 对比总结

| 能力类型 | Entity | 含义 | StateProjector 行为 |
|---------|--------|------|-------------------|
| **Write** | `block.block_id` | 被修改的块 | 更新 Block 状态 |
| **Read** | `cmd.editor_id` | 读取者 | 记录审计日志，不修改 Block |

### 2. 使用 #[capability] 宏定义能力

**【推荐】使用宏简化能力定义**

```rust
use capability_macros::capability;
use crate::capabilities::core::{CapResult, create_event};
use crate::models::{Block, Command, Event};

/// Handler for markdown.write capability.
///
/// Writes markdown content to a markdown block's contents.markdown field.
#[capability(id = "markdown.write", target = "markdown")]
fn handle_markdown_write(cmd: &Command, block: &Block) -> CapResult<Vec<Event>> {
    // 1. 解析 payload
    let payload: MarkdownWritePayload = serde_json::from_value(cmd.payload.clone())
        .map_err(|e| format!("Invalid payload: {}", e))?;

    // 2. 验证
    if payload.content.is_empty() {
        return Err("Content cannot be empty".into());
    }

    // 3. 构建新状态
    let mut new_contents = serde_json::Map::new();
    new_contents.insert("markdown".to_string(), json!(payload.content));

    // 4. 创建事件
    let event = create_event(
        block.block_id.clone(),
        format!("{}/{}", cmd.editor_id, cmd.cap_id),
        json!(new_contents),
        &cmd.timestamp,
    )?;

    Ok(vec![event])
}
```

**宏自动生成：**
- `Certificator` 实现（默认检查 owner 或 grant）
- `CapabilityHandler` 实现
- 能力注册代码

### 3. Payload 类型定义

**【关键规则】每个能力必须定义 Payload 类型**

#### 为什么 Payload 类型很重要？

**问题场景：**前端和后端对 Payload 结构的理解不一致，导致运行时错误。

#### ❌ Bad Practice - 手动 JSON 解析

**后端：**
```rust
// ❌ 手动从 JSON 提取字段
#[capability(id = "markdown.write", target = "markdown")]
fn handle_markdown_write(cmd: &Command, block: Option<&Block>) -> CapResult<Vec<Event>> {
    // 手动解析
    let content = cmd.payload.get("content")
        .and_then(|v| v.as_str())
        .ok_or("Missing 'content' in payload")?;

    // ... 实现逻辑
}
```

**前端：**
```typescript
// ❌ 手动定义接口（可能与后端不一致）
interface WriteBlockPayload {
  content: { type: 'text', data: string }  // ← 错误的结构！
}

await TauriClient.block.executeCommand(fileId, {
  // ... 其他字段 ...
  payload: {
    content: { type: 'text', data: 'Hello' }  // ← 嵌套对象，与后端期望不符
  },
});
```

**运行时错误：**
```
Error: Missing 'content' in payload
```

**问题原因：**
- 后端期望 `content` 是 `String`
- 前端发送的是 `{ type: 'text', data: 'Hello' }`
- 类型不匹配，但 TypeScript 无法检测（因为 payload 是 `any`）

#### ✅ Good Practice - 类型化 Payload

**后端：定义 Payload 类型**

```rust
// src/extensions/markdown/mod.rs
use serde::{Deserialize, Serialize};
use specta::Type;

/// Payload for markdown.write capability
///
/// This payload is used to write markdown content to a markdown block.
/// The content is stored directly as a string in the block's contents.markdown field.
#[derive(Debug, Clone, Serialize, Deserialize, Type)]
pub struct MarkdownWritePayload {
    /// The markdown content to write to the block
    pub content: String,  // ← 直接是 String，不是嵌套对象
}
```

**在 lib.rs 中注册类型：**

```rust
#[cfg(debug_assertions)]
let specta_builder = tauri_specta::Builder::<tauri::Wry>::new()
    // ... 命令注册 ...
    .typ::<extensions::markdown::MarkdownWritePayload>()  // ← 注册类型
    // ...
```

**后端处理器使用类型化 Payload：**

```rust
// src/extensions/markdown/markdown_write.rs
use super::MarkdownWritePayload;

#[capability(id = "markdown.write", target = "markdown")]
fn handle_markdown_write(cmd: &Command, block: Option<&Block>) -> CapResult<Vec<Event>> {
    // ✅ 类型安全的反序列化
    let payload: MarkdownWritePayload = serde_json::from_value(cmd.payload.clone())
        .map_err(|e| format!("Invalid payload: {}", e))?;

    // ✅ 直接使用字段，TypeScript 类型检查确保正确
    let content = payload.content;

    // ... 实现逻辑
}
```

**前端自动获得类型：**

```typescript
// src/bindings.ts (自动生成)
export type MarkdownWritePayload = {
  content: string  // ← 自动生成，与 Rust 一致
}
```

**前端使用类型化 Payload：**

```typescript
// ⚠️ 注意：以下示例仅用于说明类型系统
// 实际开发中应该在 Zustand Actions 中使用 TauriClient
// 组件不应该直接调用 TauriClient

import { TauriClient } from '@/lib/tauri-client';
import type { MarkdownWritePayload } from '@/bindings';

// ✅ TypeScript 类型检查确保 Payload 正确
const payload: MarkdownWritePayload = {
  content: 'Hello World'  // ← 直接是 string
};

// 如果写错了，TypeScript 会在编译时报错
const wrongPayload: MarkdownWritePayload = {
  content: { type: 'text', data: 'Hello' }  // ❌ 编译错误
  // Error: Type '{ type: string; data: string; }' is not assignable to type 'string'
};

// ⚠️ 以下代码应该在 Zustand Actions 中执行，不是在组件中
await TauriClient.block.executeCommand(fileId, {
  // ... 其他字段 ...
  payload: payload as any,
});
```

**✅ 实际开发中的正确做法：**

```typescript
// src/lib/app-store.ts
import { TauriClient } from '@/lib/tauri-client';
import type { MarkdownWritePayload } from '@/bindings';

export const useAppStore = create<AppStore>((set) => ({
  writeMarkdown: async (fileId: string, blockId: string, content: string) => {
    // 在 Action 中使用类型化 Payload
    const payload: MarkdownWritePayload = { content };

    await TauriClient.block.executeCommand(fileId, {
      // ...
      payload: payload as any,
    });

    // 更新 Store 状态...
  },
}));

// 组件中调用 Action
function MyComponent() {
  const writeMarkdown = useAppStore((state) => state.writeMarkdown);
  await writeMarkdown(fileId, blockId, 'Hello World');
}
```

#### 对比总结

| 方式 | 后端解析 | 前端类型 | 错误检测时机 | 维护成本 |
|------|---------|---------|-------------|---------|
| **手动解析** | `payload.get("field")` | 手动定义接口 | **运行时**（用户遇到错误） | 高（需手动同步） |
| **类型化 Payload** | `serde_json::from_value` | 自动生成 | **编译时**（TypeScript 检查） | 低（自动同步） |

#### Payload 定义位置规则

- **扩展专用** Payload → `src/extensions/{extension_name}/mod.rs`
- **核心** Payload → `src/models/payloads.rs`

这样可以保持扩展模块化和自包含。

### 4. 在 CapabilityRegistry 中注册

```rust
// src/capabilities/registry.rs
use crate::extensions::markdown::handle_markdown_write;

impl CapabilityRegistry {
    pub fn register_extensions(&mut self) {
        // Markdown 扩展
        self.register_capability(handle_markdown_write());

        // 其他扩展...
    }
}
```

### 5. CBAC 授权系统详解

**基于能力的访问控制 (Capability-Based Access Control)**

Elfiee 使用 CBAC 模型，而不是传统的 RBAC（基于角色）或 ACL（访问控制列表）。

#### 核心概念

```rust
pub struct Grant {
    pub editor_id: String,   // 谁被授权
    pub cap_id: String,      // 被授权的能力
    pub block_id: String,    // 目标块（或 "*" 表示所有块）
}
```

#### 授权规则

**规则 1：Block Owner 总是被授权**

```rust
// 在 Certificator 中
if block.owner == cmd.editor_id {
    return Ok(());  // Owner 总是有权限
}
```

**规则 2：非 Owner 需要显式 Grant**

```rust
// 检查 GrantsTable
let has_grant = grants_table.has_grant(
    &cmd.editor_id,
    &cmd.cap_id,
    &block.block_id,
);

if !has_grant {
    return Err(format!(
        "Permission denied: {} does not have {} on block {}",
        cmd.editor_id, cmd.cap_id, block.block_id
    ));
}
```

**规则 3：Wildcard Grant ("*") 适用于所有块**

```rust
// 检查通配符 Grant
let has_wildcard = grants_table.has_grant(
    &cmd.editor_id,
    &cmd.cap_id,
    "*",  // 对所有块有效
);
```

#### 完整的授权检查流程

```rust
/// Certificator 实现（由 #[capability] 宏自动生成）
pub fn check_authorization(
    cmd: &Command,
    block: Option<&Block>,
    grants_table: &GrantsTable,
) -> CapResult<()> {
    let block = block.ok_or("Block not found")?;

    // 1. Owner 总是被授权
    if block.owner == cmd.editor_id {
        return Ok(());
    }

    // 2. 检查特定 Block 的 Grant
    if grants_table.has_grant(&cmd.editor_id, &cmd.cap_id, &block.block_id) {
        return Ok(());
    }

    // 3. 检查 Wildcard Grant
    if grants_table.has_grant(&cmd.editor_id, &cmd.cap_id, "*") {
        return Ok(());
    }

    // 4. 拒绝访问
    Err(format!(
        "Permission denied: {} does not have {} on block {}",
        cmd.editor_id, cmd.cap_id, block.block_id
    ))
}
```

#### 授权测试示例

**测试 1：Owner 总是被授权**

```rust
#[tokio::test]
async fn test_authorization_owner() {
    let registry = CapabilityRegistry::new();
    let grants_table = GrantsTable::new();

    let block = Block {
        block_id: "block-1".to_string(),
        name: "Test Block".to_string(),
        block_type: "markdown".to_string(),
        contents: json!({}),
        children: HashMap::new(),
        owner: "alice".to_string(),  // ← Alice 是 Owner
        metadata: BlockMetadata::default(),
    };

    let cmd = Command {
        cmd_id: "cmd-1".to_string(),
        editor_id: "alice".to_string(),  // ← Alice 执行命令
        cap_id: "markdown.write".to_string(),
        block_id: "block-1".to_string(),
        payload: json!({ "content": "Hello" }),
        timestamp: Utc::now().to_rfc3339(),
    };

    // ✅ Owner 总是被授权
    let result = registry.execute(&cmd, Some(&block), &grants_table).await;
    assert!(result.is_ok());
}
```

**测试 2：非 Owner 无授权时被拒绝**

```rust
#[tokio::test]
async fn test_authorization_denied() {
    let registry = CapabilityRegistry::new();
    let grants_table = GrantsTable::new();  // ← 空的 GrantsTable

    let block = Block {
        block_id: "block-1".to_string(),
        name: "Test Block".to_string(),
        block_type: "markdown".to_string(),
        contents: json!({}),
        children: HashMap::new(),
        owner: "alice".to_string(),  // ← Alice 是 Owner
        metadata: BlockMetadata::default(),
    };

    let cmd = Command {
        cmd_id: "cmd-1".to_string(),
        editor_id: "bob".to_string(),  // ← Bob 不是 Owner
        cap_id: "markdown.write".to_string(),
        block_id: "block-1".to_string(),
        payload: json!({ "content": "Hello" }),
        timestamp: Utc::now().to_rfc3339(),
    };

    // ❌ Bob 没有授权
    let result = registry.execute(&cmd, Some(&block), &grants_table).await;
    assert!(result.is_err());
    assert!(result.unwrap_err().contains("Permission denied"));
}
```

**测试 3：非 Owner 有授权后可执行**

```rust
#[tokio::test]
async fn test_authorization_granted() {
    let registry = CapabilityRegistry::new();
    let mut grants_table = GrantsTable::new();

    // ✅ 授予 Bob 权限
    grants_table.add_grant(Grant {
        editor_id: "bob".to_string(),
        cap_id: "markdown.write".to_string(),
        block_id: "block-1".to_string(),  // 或 "*" 表示所有块
    });

    let block = Block {
        block_id: "block-1".to_string(),
        name: "Test Block".to_string(),
        block_type: "markdown".to_string(),
        contents: json!({}),
        children: HashMap::new(),
        owner: "alice".to_string(),  // ← Alice 是 Owner
        metadata: BlockMetadata::default(),
    };

    let cmd = Command {
        cmd_id: "cmd-1".to_string(),
        editor_id: "bob".to_string(),  // ← Bob 有授权
        cap_id: "markdown.write".to_string(),
        block_id: "block-1".to_string(),
        payload: json!({ "content": "Hello" }),
        timestamp: Utc::now().to_rfc3339(),
    };

    // ✅ Bob 有授权，可以执行
    let result = registry.execute(&cmd, Some(&block), &grants_table).await;
    assert!(result.is_ok());
}
```

#### 授权事件

授权本身也通过事件记录：

**Grant 事件：**
```rust
Event {
    event_id: "event-1",
    entity: "bob",           // 被授权的 Editor
    attribute: "alice/core.grant",  // Alice 授予权限
    value: {
        "cap_id": "markdown.write",
        "block_id": "block-1"
    },
    timestamp: {...},
    created_at: "2025-01-05T12:00:00Z",
}
```

**Revoke 事件：**
```rust
Event {
    event_id: "event-2",
    entity: "bob",           // 被撤销的 Editor
    attribute: "alice/core.revoke",  // Alice 撤销权限
    value: {
        "cap_id": "markdown.write",
        "block_id": "block-1"
    },
    timestamp: {...},
    created_at: "2025-01-05T13:00:00Z",
}
```

---

## 扩展开发

### 1. 创建新扩展

**步骤：**

1. **创建扩展目录**

```bash
mkdir -p src/extensions/my_extension
```

2. **定义模块** (`src/extensions/my_extension/mod.rs`)

```rust
/// My Extension for Elfiee.
///
/// Description of what this extension does.
///
/// ## Capabilities
///
/// - `my_extension.read`: Read data from blocks
/// - `my_extension.write`: Write data to blocks

// 子模块
pub mod my_capability;

// Payload 类型
use serde::{Deserialize, Serialize};
use specta::Type;

/// Payload for my_extension.write
#[derive(Debug, Clone, Serialize, Deserialize, Type)]
pub struct MyWritePayload {
    pub data: String,
}

// 导出能力处理器
pub use my_capability::handle_my_write;

#[cfg(test)]
mod tests {
    // 测试代码
}
```

3. **实现能力处理器** (`src/extensions/my_extension/my_capability.rs`)

```rust
use capability_macros::capability;
use crate::capabilities::core::{CapResult, create_event};
use crate::models::{Block, Command, Event};
use super::MyWritePayload;
use serde_json::json;

/// Handler for my_extension.write capability.
#[capability(id = "my_extension.write", target = "my_block_type")]
fn handle_my_write(cmd: &Command, block: &Block) -> CapResult<Vec<Event>> {
    // 解析 payload
    let payload: MyWritePayload = serde_json::from_value(cmd.payload.clone())
        .map_err(|e| format!("Invalid payload: {}", e))?;

    // 验证
    if payload.data.is_empty() {
        return Err("Data cannot be empty".into());
    }

    // 构建新状态
    let mut new_contents = serde_json::Map::new();
    new_contents.insert("data".to_string(), json!(payload.data));

    // 创建事件
    let event = create_event(
        block.block_id.clone(),
        format!("{}/{}", cmd.editor_id, cmd.cap_id),
        json!(new_contents),
        &cmd.timestamp,
    )?;

    Ok(vec![event])
}
```

4. **在 extensions/mod.rs 中导出**

```rust
// src/extensions/mod.rs
pub mod markdown;
pub mod code;
pub mod directory;
pub mod my_extension; // 添加新扩展
```

5. **在 CapabilityRegistry 中注册**

```rust
// src/capabilities/registry.rs
use crate::extensions::my_extension::handle_my_write;

impl CapabilityRegistry {
    pub fn register_extensions(&mut self) {
        // 已有扩展...

        // 新扩展
        self.register_capability(handle_my_write());
    }
}
```

6. **在 lib.rs 中注册类型**

```rust
#[cfg(debug_assertions)]
let specta_builder = tauri_specta::Builder::<tauri::Wry>::new()
    // ... 命令 ...
    .typ::<extensions::my_extension::MyWritePayload>()
    // ... 其他类型 ...
```

### 2. 扩展开发最佳实践

**模块化：**
- 每个能力一个文件
- Payload 类型定义在 mod.rs 中
- 导出所有能力处理器

**文档：**
- 详细的文档注释
- Payload 字段说明
- 示例用法

**测试：**
- 单元测试能力处理器
- 集成测试完整命令流程
- 测试授权检查

---

## 事件溯源

### 1. 事件结构

```rust
pub struct Event {
    pub event_id: String,                        // 事件唯一 ID
    pub entity: String,                          // 实体 ID (block_id 或 editor_id)
    pub attribute: String,                       // "{editor_id}/{cap_id}"
    pub value: JsonValue,                        // 状态变更数据
    pub timestamp: HashMap<String, u64>,         // 向量时钟
    pub created_at: String,                      // 创建时间戳 (ISO 8601)
}
```

### 2. 创建事件

**使用工具函数：**

```rust
use crate::capabilities::core::create_event;
use serde_json::json;

let event = create_event(
    block.block_id.clone(),                          // Entity
    format!("{}/{}", cmd.editor_id, cmd.cap_id),     // Attribute
    json!(new_contents),                             // Value
    &cmd.timestamp,                                  // Timestamp (vector clock)
)?;
```

### 3. 事件存储

**EventStore** 使用 SQLite 存储事件：

```sql
CREATE TABLE events (
    event_id TEXT PRIMARY KEY,
    entity TEXT NOT NULL,
    attribute TEXT NOT NULL,
    value TEXT NOT NULL,        -- JSON
    timestamp TEXT NOT NULL,    -- JSON (vector clock)
    created_at TEXT NOT NULL    -- ISO 8601
);
```

**插入事件：**

```rust
event_store.append_events(&events).await?;
```

### 4. 状态投影

**StateProjector** 通过重放事件构建状态：

```rust
let mut projector = StateProjector::new();

// 重放所有事件
for event in events {
    projector.apply_event(&event)?;
}

// 获取最终状态
let blocks = projector.get_blocks();
let editors = projector.get_editors();
let grants = projector.get_grants();
```

### 5. 向量时钟与并发控制

#### 为什么需要向量时钟？

在多用户协作环境中，事件可能并发发生：
- Alice 和 Bob 同时编辑同一个块
- 需要检测冲突并拒绝过期的命令

#### 向量时钟结构

```rust
pub struct Event {
    pub timestamp: HashMap<String, u64>,  // ← 向量时钟
    // editor_id → transaction_count
}
```

**示例：**
```rust
timestamp: {
    "alice": 5,  // Alice 的第 5 个操作
    "bob": 3,    // Bob 的第 3 个操作
}
```

#### 向量时钟比较

**事件 A 先于事件 B（A → B）：**
```rust
// A 的所有计数器 ≤ B 的计数器，且至少有一个 <
A.timestamp = { "alice": 3, "bob": 2 }
B.timestamp = { "alice": 4, "bob": 2 }  // alice 的计数器增加了
// A → B (A 先于 B)
```

**并发事件（A || B）：**
```rust
A.timestamp = { "alice": 3, "bob": 2 }
B.timestamp = { "alice": 2, "bob": 3 }  // 无法比较
// A || B (并发)
```

#### 冲突检测

**Engine 在执行命令前检查向量时钟：**

```rust
// 伪代码
fn execute_command(cmd: Command) -> Result<Vec<Event>, String> {
    // 1. 获取当前状态的向量时钟
    let current_vc = state.get_vector_clock();

    // 2. 比较命令的向量时钟
    if !is_compatible(cmd.timestamp, current_vc) {
        return Err("Conflict detected: stale command".to_string());
    }

    // 3. 执行命令
    let events = capability.handler(&cmd, block)?;

    // 4. 更新向量时钟
    for event in &events {
        // 增加当前 editor 的计数器
        event.timestamp.insert(
            cmd.editor_id.clone(),
            current_vc.get(&cmd.editor_id).unwrap_or(&0) + 1,
        );
    }

    Ok(events)
}
```

#### create_event 自动处理向量时钟

```rust
pub fn create_event(
    entity: String,
    attribute: String,
    value: JsonValue,
    base_timestamp: &str,  // ← 命令的时间戳（用于向量时钟）
) -> CapResult<Event> {
    // 解析基础向量时钟
    let timestamp = parse_vector_clock(base_timestamp)?;

    Ok(Event {
        event_id: Uuid::new_v4().to_string(),
        entity,
        attribute,
        value,
        timestamp,  // ← 继承并递增
        created_at: Utc::now().to_rfc3339(),
    })
}
```

#### 实际使用

**能力处理器中：**

```rust
#[capability(id = "markdown.write", target = "markdown")]
fn handle_markdown_write(cmd: &Command, block: Option<&Block>) -> CapResult<Vec<Event>> {
    // ... 解析 payload ...

    // ✅ create_event 自动处理向量时钟
    let event = create_event(
        block.block_id.clone(),
        format!("{}/{}", cmd.editor_id, cmd.cap_id),
        json!(new_contents),
        &cmd.timestamp,  // ← 传入命令的时间戳
    )?;

    Ok(vec![event])
}
```

**前端构造命令时：**

```typescript
const cmd: Command = {
  cmd_id: crypto.randomUUID(),
  editor_id: editorId,
  cap_id: 'markdown.write',
  block_id: blockId,
  payload: payload as any,
  timestamp: new Date().toISOString(),  // ← ISO 8601 格式
};
```

**【重要】前端不需要手动管理向量时钟**，后端 Engine 会自动处理。

---

## 类型安全与自动生成

### 1. 类型定义规范

**【规则】所有前端可见类型必须：**
1. 添加 `#[derive(Serialize, Deserialize)]`
2. 添加 `#[derive(Type)]` (来自 specta)
3. 写清晰的文档注释

**示例：**

```rust
use serde::{Deserialize, Serialize};
use specta::Type;

/// Represents a block in the Elfiee system.
///
/// Blocks are the fundamental data units, similar to documents or notes.
#[derive(Debug, Clone, Serialize, Deserialize, Type)]
pub struct Block {
    /// Unique identifier for this block
    pub block_id: String,

    /// Display name of the block
    pub name: String,

    /// Type of the block (e.g., "markdown", "code", "directory")
    pub block_type: String,

    /// Block contents (JSON format, structure depends on block_type)
    pub contents: JsonValue,

    /// Relations to other blocks (relation_type -> [block_ids])
    pub children: HashMap<String, Vec<String>>,

    /// Owner editor ID (has all permissions)
    pub owner: String,

    /// Block metadata
    pub metadata: BlockMetadata,
}
```

### 2. 类型注册清单

**在 lib.rs 中注册所有类型：**

```rust
let specta_builder = tauri_specta::Builder::<tauri::Wry>::new()
    .commands(/* ... */)
    // Core 模型
    .typ::<models::Block>()
    .typ::<models::Command>()
    .typ::<models::Event>()
    .typ::<models::Editor>()
    .typ::<models::Grant>()
    // Core Payload 类型
    .typ::<models::CreateBlockPayload>()
    .typ::<models::LinkBlockPayload>()
    .typ::<models::GrantPayload>()
    // 扩展 Payload 类型
    .typ::<extensions::markdown::MarkdownWritePayload>()
    .typ::<extensions::code::CodeWritePayload>()
    .typ::<extensions::directory::DirectoryCreatePayload>()
    // 其他类型
    .typ::<commands::FileMetadata>()
    ;
```

### 3. 类型生成流程

```
Rust 类型定义 (#[derive(Type)])
    ↓
lib.rs 注册 (.typ::<MyType>())
    ↓
pnpm tauri dev 运行
    ↓
tauri-specta 扫描类型
    ↓
生成 src/bindings.ts
    ↓
前端导入使用 (import { type MyType } from '@/bindings')
```

---

## 测试规范

### 1. 单元测试

**测试能力处理器：**

```rust
#[cfg(test)]
mod tests {
    use super::*;
    use crate::models::{Block, Command};
    use serde_json::json;

    #[test]
    fn test_markdown_write() {
        let block = Block {
            block_id: "block-1".to_string(),
            name: "Test Block".to_string(),
            block_type: "markdown".to_string(),
            contents: json!({}),
            children: HashMap::new(),
            owner: "alice".to_string(),
            metadata: BlockMetadata::default(),
        };

        let cmd = Command {
            cmd_id: "cmd-1".to_string(),
            editor_id: "alice".to_string(),
            cap_id: "markdown.write".to_string(),
            block_id: "block-1".to_string(),
            payload: json!({
                "content": "# Hello World"
            }),
            timestamp: chrono::Utc::now().to_rfc3339(),
        };

        let result = handle_markdown_write(&cmd, &block);
        assert!(result.is_ok());

        let events = result.unwrap();
        assert_eq!(events.len(), 1);
        assert_eq!(events[0].entity, "block-1");
    }
}
```

### 2. 集成测试

**测试完整命令流程：**

```rust
#[tokio::test]
async fn test_create_and_write_block() {
    // 创建临时文件
    let temp_dir = tempfile::tempdir().unwrap();
    let file_path = temp_dir.path().join("test.elf");

    // 创建文件
    let file_id = create_file(file_path.to_str().unwrap().to_string())
        .await
        .unwrap();

    // 打开文件
    let file_id = open_file(file_path.to_str().unwrap().to_string())
        .await
        .unwrap();

    // 创建块
    let create_cmd = Command {
        cmd_id: Uuid::new_v4().to_string(),
        editor_id: "system".to_string(),
        cap_id: "core.create".to_string(),
        block_id: "*".to_string(),
        payload: json!({
            "name": "Test Block",
            "block_type": "markdown"
        }),
        timestamp: Utc::now().to_rfc3339(),
    };

    let events = execute_command(file_id.clone(), create_cmd)
        .await
        .unwrap();

    let block_id = events[0].entity.clone();

    // 写入内容
    let write_cmd = Command {
        cmd_id: Uuid::new_v4().to_string(),
        editor_id: "system".to_string(),
        cap_id: "markdown.write".to_string(),
        block_id: block_id.clone(),
        payload: json!({
            "content": "# Hello World"
        }),
        timestamp: Utc::now().to_rfc3339(),
    };

    let result = execute_command(file_id.clone(), write_cmd).await;
    assert!(result.is_ok());

    // 验证状态
    let block = get_block(file_id, block_id, None).await.unwrap();
    assert_eq!(block.block_type, "markdown");
}
```

### 3. 授权测试

**测试 CBAC 权限检查：**

```rust
#[tokio::test]
async fn test_permission_denied() {
    let file_id = create_test_file().await;

    // 创建块（owner: alice）
    let block_id = create_block_as_editor(file_id.clone(), "alice").await;

    // Bob 尝试写入（没有授权）
    let write_cmd = Command {
        cmd_id: Uuid::new_v4().to_string(),
        editor_id: "bob".to_string(),
        cap_id: "markdown.write".to_string(),
        block_id: block_id.clone(),
        payload: json!({
            "content": "Hacked!"
        }),
        timestamp: Utc::now().to_rfc3339(),
    };

    let result = execute_command(file_id, write_cmd).await;
    assert!(result.is_err());
    assert!(result.unwrap_err().contains("Permission denied"));
}
```

---

## 常见错误与陷阱

### ❌ 错误 1：忘记在 lib.rs 注册命令或类型
**后果：** 前端报错 "Command not found" 或类型缺失
**解决：** 在 lib.rs 的 **debug 和 release** 模式都要注册

### ❌ 错误 2：Read/Write 能力的 Entity 错误
```rust
// ❌ 错误：Read 能力使用了 block_id
let event = create_event(
    block.block_id.clone(),  // ❌ Read 应该用 editor_id
    // ...
);

// ✅ 正确：根据能力类型选择 Entity
// Write: entity = block.block_id (被修改的块)
// Read: entity = cmd.editor_id (读取者)
```

### ❌ 错误 3：手动解析 Payload 而不用类型
```rust
// ❌ 错误：手动解析容易出错
let content = cmd.payload.get("content")
    .and_then(|v| v.as_str())
    .ok_or("Missing 'content'")?;

// ✅ 正确：使用类型化 Payload
let payload: MarkdownWritePayload = serde_json::from_value(cmd.payload.clone())?;
let content = payload.content;
```

### ❌ 错误 4：直接修改状态而不用 Event
```rust
// ❌ 错误：违反事件溯源原则
block.contents = json!({ "new": "data" });

// ✅ 正确：通过 Event 记录变更
let event = create_event(
    block.block_id.clone(),
    format!("{}/{}", cmd.editor_id, cmd.cap_id),
    json!(new_contents),
    &cmd.timestamp,
)?;
```

### ❌ 错误 5：忘记测试授权
**后果：** 权限漏洞，任何人都能修改
**解决：** 添加三个测试：Owner 授权、非 Owner 拒绝、非 Owner 有 Grant

---

## 总结

### 后端开发者必须遵守的规则

1. **【严格】** 所有前端可调用功能必须定义为 Tauri Command
2. **【严格】** 使用 `#[specta]` 宏生成类型绑定
3. **【严格】** 在 lib.rs 中注册所有命令和类型
4. **【严格】** 遵循事件溯源架构，通过 Event 记录状态变更
5. **【推荐】** 使用 `#[capability]` 宏定义能力
6. **【推荐】** 为每个能力定义 Payload 类型
7. **【推荐】** 在能力处理器中使用 `serde_json::from_value` 解析 Payload
8. **【推荐】** 编写单元测试和集成测试
9. **【推荐】** 测试授权检查和错误处理

### 核心开发流程

```
1. 定义数据模型（添加 #[derive(Type)]）
2. 定义 Payload 类型（添加 #[derive(Type)]）
3. 实现能力处理器（使用 #[capability] 宏）
4. 在 CapabilityRegistry 中注册能力
5. 定义 Tauri Command（添加 #[tauri::command] + #[specta]）
6. 在 lib.rs 中注册命令和类型（debug + release）
7. 运行 pnpm tauri dev 生成类型绑定
8. 前端使用生成的类型和命令
```

### 添加新功能时

当需要添加新功能时：
1. 确定是否需要新的 Tauri Command
2. 确定是否需要新的 Capability
3. 定义 Payload 类型（如果需要）
4. 实现能力处理器
5. 注册命令和类型
6. 编写测试
7. 更新文档

---

## 🏁 自查清单 (Checklist)

> 在提交代码前，请对照以下事项进行自查。AI 助手必须严格遵守。

- [ ] **工具强制**: 是否使用了 `elfiee-ext-gen create` 进行初始化？(禁止手动拼写模块文件)
- [ ] **命令注册**: 在 `src-tauri/src/lib.rs` 中，是否在 **Debug 和 Release** 块中都注册了命令？
- [ ] **Payload 类型**: 所有的 Capability 是否都定义了独立的 Payload 结构体并派生了 `Type`？
- [ ] **Entity ID**: 
    - Write 能力的 Entity 是 `block_id` 吗？
    - Read 能力的 Entity 是 `editor_id` 吗？
- [ ] **状态变更**: 是否通过 `create_event` 生成事件并返回？(禁止在 Handler 里直接修改内存对象)
- [ ] **测试覆盖**: `tests.rs` 中是否包含 Payload 反序列化、基本功能和授权测试？

**更多详细规则请参考：[AI_INDEX.md](./AI_INDEX.md)**

---

**相关文档：**
- [前端开发规范](./前端开发规范.md) - 前端开发详细指南
- [开发流程](./开发流程.md) - 完整开发流程和协作规范
- [CLAUDE.md](../../CLAUDE.md) - 项目总览和 AI 辅助开发
- [EXTENSION_DEVELOPMENT.md](../../guides/EXTENSION_DEVELOPMENT.md) - 扩展开发详细指南
