 ...
 */
async asyncInitTerminal(payload: TerminalInitPayload) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("async_init_terminal", { payload }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
async writeToPty(payload: TerminalWritePayload) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("write_to_pty", { payload }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 * Resize the PTY.
 */
async resizePty(payload: TerminalResizePayload) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("resize_pty", { payload }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 * Close a PTY session.
 */
async closeTerminalSession(fileId: string, blockId: string, editorId: string) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("close_terminal_session", { fileId, blockId, editorId }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
target_block?: string }
export type TerminalInitPayload = { cols: number; rows: number; block_id: string; editor_id: string; file_id: string; cwd: string | null }
export type TerminalResizePayload = { cols: number; rows: number; block_id: string; file_id: string; editor_id: string }
/**
 ...
 */
saved_at: string }
export type TerminalWritePayload = { data: string; block_id: string; file_id: string; editor_id: string }
/**
