...
import {
  ...
  type GrantPayload,
  type RevokePayload,
  type TerminalExecutePayload,
  type TerminalReadPayload,
...
export class TerminalOperations {
  private static async getLatestTerminalEvent(fileId: string, blockId: string) {
    ...
    const terminalEvents = events.filter(
      (event) =>
        event.entity === blockId &&
        (event.attribute.endsWith('/terminal.execute') ||
          event.attribute.endsWith('/terminal.write') ||
          event.attribute.endsWith('/terminal.read'))
    )
   ...
   * @param editorId - Editor performing the command
   * @returns Events generated by the command
   */
  ...
  static async getTerminalHistory(
    ...
    const eventValue = (latestEvent?.value as { contents?: unknown }) ?? {}
    const contents = (eventValue.contents as any) ?? {}
  ...
  static async getTerminalState(
    ...
    const contents = (eventValue.contents as any) ?? {}
    return {
      history: (contents.history as TerminalCommandEntry[] | undefined) ?? [],
      currentDirectory: contents.current_directory || 'block-root',
      contents: contents ?? {},
    }
  }

  /**
   * 读取终端状态（使用terminal.read capability）
   * @param fileId - File ID
   * @param blockId - Terminal block ID
   * @param editorId - Editor performing the read
   * @returns Empty events array (read operations don't generate events)
   */
  static async readTerminal(
    ...
    const payload: TerminalReadPayload = {}
    const cmd = createCommand(
      editorId,
      'terminal.read',
      blockId,
  ...
  /**
   * 向终端写入内容（使用terminal.write capability）
   * @param fileId - File ID
   * @param blockId - Terminal block ID
   * @param data - Content to write to terminal
   * @param editorId - Editor performing the write
   * @returns Events generated by the write operation
   */
  static async writeToTerminal(
    fileId: string,
    blockId: string,
    data: JsonValue,
    editorId: string = DEFAULT_EDITOR_ID
